  ...
  for (linenum in which(speciallines)) {
    ...
    if (grepl("^#' \\[", linecontent)) {
      ...
      if (is.na(fig.scale)) {
        ...
        fig.scale <- as.numeric(substr(fig.scale, 2, nchar(fig.scale)))
      }

      # Getting and stripping the figure name
      fig.name <- stringr::str_extract(comment, "//.*$")
      if (!is.na(fig.name)) {
        fig.name <- trimws(substr(fig.name, 3, nchar(fig.name)))
      } else {
        fig.name <- NULL
      }
...
knit_markdown <- function(markdown_file,
  ...
  # Set code toggle
  if (codetoggle) {
    codetogglediv <- "<div class='headerlink' id='codetoggle'></div>"
  } else {
    codetogglediv <- NULL
  }

...
get_page_metadate <- function(pagepath) {
    ...
    pagedates <- pagemeta
    pagedates[!is.na(pagemeta)] <- sapply(pagemeta[!is.na(pagemeta)], readLines) |> unname()
    as.Date(pagedates)
...
get_page_stats <- function() {
    ...
    project_files$codefile_mtime <- file.info(project_files$codefile)$mtime |> as.Date()
...
update_page_stats <- function() {
    ...
    jsonlite::write_json(page_stats, "../../library/pagestats.js", auto_unbox = TRUE)
